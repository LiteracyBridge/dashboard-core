...
public class TbData2 {
    ...
    static {
        ...
        options.addOption("d", true,
            "Directory into which to accumulate various key:value log files (as *.kvp).");

        options.addOption("x", false, "Do not write to SQL db.");
    ...
    /**
     * Import stats from one zip file.
     * @param contentUsageUpdateProcess The helper that does the actual work.
     * @param zipFile to be imported
     * @param validationParameters, force (to ignore errors), strict (tighter validation; would not
     *                              succeed), and format (never used)
     * @param result to accumulate status as files are processed
     * @return the "context" from the import call
     * @throws Exception if the import throws one.
     */
    private static ContentUsageUpdateProcess.UpdateUsageContext importZip(
        ...
        context = contentUsageUpdateProcess.createInitialContext(
            is,
            tempDir,
            "Commandline Tool",
            "Non Specific",
    ...
    private void go(String[] args) throws Exception {
        ...
        for (File fileToProcess : filesToProcess) {
            ...
            ContentUsageUpdateProcess.UpdateUsageContext context;
            context = importZip(contentUsageUpdateProcess, fileToProcess, validationParameters, result);
        ...
        if (cmd.hasOption("d")) {
            contentUsageUpdateProcess.closeGlobalDirectory();
        }
